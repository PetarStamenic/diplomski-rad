...
public class DatabaseControler {
    ...
    public ResponseEntity<String > add(@PathVariable(value = "table")String table,@PathVariable(value = "database")String database, @RequestBody LinkedHashMap body){
    ...
    public ResponseEntity<Object> find(@PathVariable(value = "table")String table,@PathVariable(value = "database")String database,@PathVariable(value = "requests")String requests){
    ...
    public ResponseEntity<Object> findAll(@PathVariable(value = "table")String table,@PathVariable(value = "database")String database){
        Object o = databaseService.listAllFromTable(database,table);
        return new ResponseEntity<>(o,HttpStatus.OK);
    }

    @GetMapping("/{database}/{table}/{requests}/{updates}")
    public ResponseEntity<Object> update(@PathVariable(value = "table")String table,@PathVariable(value = "database")String database,@PathVariable(value = "requests")String requests,@PathVariable(value = "updates")String updates){
        List<RequestDTO> requestDTOS = convertRequestsToList(requests);
    ...
    public ResponseEntity<Object> delete(@PathVariable(value = "table")String table,@PathVariable(value = "database")String database,@PathVariable(value = "requests")String requests){
        List<RequestDTO> requestDTOS = convertRequestsToList(requests);
        if(requestDTOS == null || requestDTOS.isEmpty())
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        if(requests.contains("&")){
